package com.ly.yipush.thread;
/*事例
 * ThreadPool.getInstance().start(pr_new Runnable() {
...
public class ThreadPool 
{
    ...
    protected synchronized void repool(PooledThread repoolingThread)
    {
        if (!shutDown_) 
        {
            Log.e(TAG, "ThreadPool.repool() : repooling ");
        ...
        {
            Log.e(TAG, "ThreadPool.repool() : Destroying incoming thread.");
            repoolingThread.shutDown();//关闭线程
        }
        Log.e(TAG, "ThreadPool.recycle() : done.");
    }
    ...
    {
        shutDown_ = true;
        Log.e(TAG, "ThreadPool : shutting down ");
        ...
        notifyAll();
        Log.e(TAG, "ThreadPool : shutdown done.");
    }
     ...
     * @param priority 优先级
     */
    public synchronized PooledThread start(Runnable target, int priority)
    {
